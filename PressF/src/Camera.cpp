...
void FlyingController::Update()
{
	...
	// this should be precalculated per object type
...
void FlyingController::HandleEvent(const SDL_Event & e)
{
	...
	if (e.type == SDL_EventType::SDL_MOUSEMOTION) {
		...
		//Vec3 vecRot = Vec3(glm::radians(front.x), glm::radians(front.y), glm::radians(front.z)) * deltaTime;
		//transform.SetRotation(front * 20.f);
		
	/*	transform.front = transform.Rotate(front);
		transform.right = glm::normalize(glm::cross(Front, WorldUp));
		transform.up = glm::normalize(glm::cross(Right, Front));*/
